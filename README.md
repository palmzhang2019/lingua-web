...
- **素材删除** — 支持批量选择并删除错误上传的素材。尚未用于学习的素材会被删除；已有学习历史关联的素材会从新学习列表中隐藏，但保留历史来源与成绩追溯。
...
以 `uv run pytest tests/` 运行测试时自动设置 `LINGUA_TESTING=1` 和临时数据库路径。详细配置见 `app/config.py`。
...
- **Material Deletion** — Select and remove mistakenly uploaded materials. Unused materials are hard-deleted; materials referenced by learning history are hidden from future selection while retaining historical provenance and results.
...
| Templates | Jinja2 (server‑rendered, no external CDN dependencies) |
| Package Manager | uv |
