...
from app.agents.generator import (
    ...
    TranslationExercise as GenTranslationExercise,
)
...
# =============================================================================
# Phase 5D-1: Manual mastered replacement for active cycle target grammar
# =============================================================================


def _find_eligible_replacement_grammar(
    db: Session, cycle: StudyCycle, withdrawn_grammar_id: int
) -> GrammarPoint | None:
    """Find an eligible replacement grammar for a withdrawn target grammar.

    Eligible conditions:
      - From the same selected material used by the active cycle.
      - mastered == false.
      - Not the other surviving current target grammar.

    Returns the first eligible candidate deterministically, or None.
    """
    # Determine the surviving target grammar id (the one NOT being withdrawn)
    surviving_id = (
        cycle.grammar_b_id if cycle.grammar_a_id == withdrawn_grammar_id
        else cycle.grammar_a_id if cycle.grammar_b_id == withdrawn_grammar_id
        else None
    )
    if surviving_id is None:
        return None

    # Get material IDs for this cycle
    cms = db.query(CycleMaterial).filter(
        CycleMaterial.cycle_id == cycle.id
    ).all()
    material_ids = [cm.material_id for cm in cms]
    ...
    # Use the existing grammar pool builder — it returns eligible (unmastered) candidates
    eligible_candidates, _ = _build_combined_grammar_pool(db, material_ids)

    # Filter out withdrawn and surviving targets
    for gp in eligible_candidates:
        if gp.id != withdrawn_grammar_id and gp.id != surviving_id:
            return gp

    return None


def _reset_attempt_slot(slot: QuestionAttempt) -> None:
    """Clear all user-interaction state from a slot, making it 'planned' again.

    This is the fixed-slot reuse semantics: the row stays, but all business
    content belonging to the withdrawn grammar is erased.
    """
    slot.question_payload_json = {"type": slot.question_payload_json.get("type", "translation")}
    slot.correct_answer = ""
...
def _remove_withdrawn_candidates(
    db: Session, cycle_id: int, withdrawn_attempt_ids: set[int]
) -> None:
    """Delete TranslationErrorCandidate records generated by the withdrawn attempts."""
    if not withdrawn_attempt_ids:
        return
    db.query(TranslationErrorCandidate).filter(
        TranslationErrorCandidate.cycle_id == cycle_id,
        TranslationErrorCandidate.source_attempt_id.in_(withdrawn_attempt_ids),
    ).delete(synchronize_session=False)


def _rollback_withdrawn_weak_points(
    db: Session, cycle_id: int, withdrawn_attempt_ids: set[int]
) -> None:
    """Reverse all current-cycle weak-point effects attributable to withdrawn attempts.

    Uses WeakPointEvent provenance for exact reversal.
    Rolls back WeakPoint.error_count, deletes WeakPoint if count reaches 0,
    and removes the corresponding WeakPointEvent records.
    """
    if not withdrawn_attempt_ids:
        return
    ...
    events = db.query(WeakPointEvent).filter(
        ...
        WeakPointEvent.source_attempt_id.in_(withdrawn_attempt_ids),
    ).all()

    for event in events:
        wp = db.query(WeakPoint).filter(
            WeakPoint.id == event.weak_point_id
        ).first()
        if wp is None:
            continue

        wp_error_count = wp.error_count or 0

        # Delete event first using raw SQL (more reliable in shared test sessions)
        db.execute(
            text("DELETE FROM weak_point_events WHERE id = :id"),
            {"id": event.id},
        )
        db.flush()

        if event.event_type == "created":
            other_events = db.query(WeakPointEvent).filter(
                WeakPointEvent.weak_point_id == wp.id,
            ).count()
            if other_events == 0:
                db.execute(
                    text("DELETE FROM weak_points WHERE id = :id"),
                    {"id": wp.id},
                )
                db.flush()
            else:
                wp.error_count = max(wp_error_count - 1, 0)
                if wp.error_count < 2:
                    wp.is_active = False
        elif event.event_type == "hit_existing":
            wp.error_count = max(wp_error_count - 1, 0)
            if wp.error_count == 0:
                other_events = db.query(WeakPointEvent).filter(
                    WeakPointEvent.weak_point_id == wp.id,
                ...
                if other_events == 0:
                    ...
                    wp.is_active = False
            elif wp.error_count < 2:
                wp.is_active = False

    db.flush()


def _execute_manual_mastered_replacement(
    db: Session,
    cycle: StudyCycle,
    withdrawn_grammar: GrammarPoint,
    replacement_grammar: GrammarPoint,
) -> bool:
    """Execute the atomic manual-mastered replacement transaction.

    Args:
        db: Database session.
        cycle: The active StudyCycle.
        withdrawn_grammar: The GrammarPoint being marked mastered and withdrawn.
        replacement_grammar: The replacement GrammarPoint.

    Returns: True on success. On failure raises an exception (caller handles rollback).
    """
    # ---- Determine target position ----
    if cycle.grammar_a_id == withdrawn_grammar.id:
        position = "grammar_a"
        surviving_grammar_id = cycle.grammar_b_id
        translation_module = "grammar_a_translation"
    elif cycle.grammar_b_id == withdrawn_grammar.id:
        position = "grammar_b"
        surviving_grammar_id = cycle.grammar_a_id
        translation_module = "grammar_b_translation"
    else:
        raise ValueError("withdrawn_grammar is not a target of this cycle")

    # ---- Identify all slots belonging to the withdrawn target ----
    all_qs = _get_sorted_cycle_questions(db, cycle.id)

    # Translation slots: by module_type
    translation_slots = [
        q for q in all_qs
        if q.module_type == translation_module
    ]

    # MC distinction slots: first 2 MC slots (0-1) for GA, next 2 (2-3) for GB
    ...
    withdrawn_attempt_ids: set[int] = set()
    all_withdrawn_slots = translation_slots + withdrawn_mc_slots

    # ---- Step 1: Reset and rebind translation slots ----
    for slot in translation_slots:
        withdrawn_attempt_ids.add(slot.id)
        _reset_attempt_slot(slot)
        slot.target_grammar_id = replacement_grammar.id

    # ---- Step 2: Reset MC distinction slots ----
    for slot in withdrawn_mc_slots:
        withdrawn_attempt_ids.add(slot.id)
        _reset_attempt_slot(slot)
        # MC slots don't have target_grammar_id set; MC generation uses
        # slot index + StudyCycle grammar ids — no rebind needed

    # ---- Step 3: Mark original grammar as mastered ----
    withdrawn_grammar.mastered = True

    # ---- Step 4: Update cycle target assignment ----
    if position == "grammar_a":
        cycle.grammar_a_id = replacement_grammar.id
    else:
        cycle.grammar_b_id = replacement_grammar.id

    # ---- Step 5: Remove candidates for withdrawn attempts ----
    _remove_withdrawn_candidates(db, cycle.id, withdrawn_attempt_ids)

    # ---- Step 6: Roll back weak points from withdrawn attempts ----
    _rollback_withdrawn_weak_points(db, cycle.id, withdrawn_attempt_ids)

    # ---- Step 7: Adjust session state to point to replacement translations ----
    state = db.query(SessionState).first()
    if state and state.current_cycle_id == cycle.id:
        # Find the first reset translation slot index
        refreshed_all = _get_sorted_cycle_questions(db, cycle.id)
        first_replacement_idx = None
        for i, q in enumerate(refreshed_all):
            if q.id in {s.id for s in translation_slots} and q.status == "planned":
                first_replacement_idx = i
                break

        if first_replacement_idx is not None:
            state.current_question_index = first_replacement_idx
            state.current_module = translation_module
        else:
            # Fallback — advance to next pending
            next_pending = _first_pending_question_index(refreshed_all)
            if next_pending is not None:
                state.current_question_index = next_pending
                state.current_module = refreshed_all[next_pending].module_type
            else:
                state.current_question_index = len(refreshed_all)
                _compute_cycle_completion(db, cycle)
        state.updated_at = datetime.datetime.utcnow()

    # ---- Step 8: Persist usage logs ----
...
def _is_current_cycle_target(
    db: Session, grammar_point_id: int
) -> tuple[bool, StudyCycle | None, str | None]:
    """Check if the given grammar is a target of the current active cycle.

    Returns (is_target, cycle, position) where position is "grammar_a" or "grammar_b".
    """
    state = db.query(SessionState).first()
    if not state or not state.current_cycle_id:
    ...
    if cycle.grammar_a_id == grammar_point_id:
        return True, cycle, "grammar_a"
    if cycle.grammar_b_id == grammar_point_id:
        return True, cycle, "grammar_b"

    return False, None, None


def _build_no_replacement_html(grammar_name: str) -> str:
    """Build a user-visible HTML message for the no-replacement case."""
    return (
        "<div class='card flash-error'>"
        "<strong>无法标记掌握：</strong>"
        f"「{grammar_name}」是当前学习循环的目标语法之一。"
        "当前选择的素材中没有可用的替换语法。"
        "请先完成当前循环，或选择其他素材后再操作。</div>"
        "<a href='/study/current' class='btn btn-primary'>返回学习</a>"
    )


def _build_replacement_html(
    withdrawn_name: str, replacement_name: str, position: str
) -> str:
    """Build a user-visible HTML message for successful replacement."""
    pos_label = "语法 A" if position == "grammar_a" else "语法 B"
    return (
        "<div class='card flash-success'>"
        f"<strong>「{withdrawn_name}」已标记为掌握。</strong><br>"
        f"{pos_label} 已替换为「{replacement_name}」。"
        "正在准备新的翻译练习…</div>"
        "<a href='/study/current' class='btn btn-primary'>继续学习</a>"
    )
